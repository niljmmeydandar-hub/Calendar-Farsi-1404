...
for (const month of calendarObject) {
	...
	for (const day of month) {
		...
		if (day[5] === true) {
			 ulCurrent.innerHTML += `<li class="empty"></li>`; 
			  liCounter++;
			  continue;
}
...
if (liCounter === 6) {
    liClass += "holiday ";
		...
		ulCurrent.innerHTML += `
<li class="${liClass}" 
    data-committee="${(day[4][0] ?? 'بدون کمیته').replace(/"/g,'')}">
    <span id="jalali">${day[0].toString().convertDigits("fa")}</span>
    <small id="miladi">${day[1]}</small>
    <small id="ghamari">${day[2].toString().convertDigits("ar")}</small>
</li>
`;
setTimeout(() => {

    const className = `date-${monthCounter}-${day[0].convertDigits("en")}`;
    const dayElement = document.getElementsByClassName(className)[0];

    if (!dayElement) return;

    dayElement.onclick = function () {
        const box = document.getElementById("committee-box");
        const title = document.getElementById("committee-title");
        const body = document.getElementById("committee-body");

        title.innerText = `${day[0].convertDigits("fa")} ${currentMonth}`;

        body.innerHTML = "";

        if (day[4].length > 0) {
            day[4].forEach(ev => {
                body.innerHTML += `<div>${ev}</div>`;
            });
        } else {
            body.innerHTML = "<div>بدون کمیته و بدون رویداد</div>";
        }
